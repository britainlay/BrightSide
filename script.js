...
if (!bookingPage) {
  console.log("BrightSide: not on booking page.");
} else {
  ...
  /*
     IMPORTANT:
     This must be a PUBLIC Mapbox token beginning
     with "pk.".

     For production, use a URL-restricted token.
  */
  ...
  const serviceDistance =
    document.getElementById("service-distance");
  ...
  function updatePrices() {
    ...
    if (!vehicle) {
      ...
      priceElements.forEach(element => {
        element.textContent = "Select Vehicle";
      });
  ...
  function startMap() {
    ...
    if (
      ...
      console.error(
        "BrightSide: Mapbox public token is missing or invalid."
      );

      showMapError(
        "The map could not load because the Mapbox token is missing or invalid."
      );
    ...
    try {
      ...
      console.error(
        "BrightSide: Map initialization failed:",
        error
      );
  ...
  function initializeMap() {
    ...
    showMapError(
      "Mapbox is still loading. Please refresh the page if the map does not appear."
    );
  ...
  /*
     Because booking.html loads Mapbox with defer,
